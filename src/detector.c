...
void train_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear,char* base) {
...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen) {
	...
	while (1) {
		...
		time = clock();
		network_predict(net, X);
		printf("%s: Predicted in %f seconds.\n", input, (clock() - time) / CLOCKS_PER_SEC);
#else
...
void run_detector(int argc, char **argv) {
	...
	else if (0 == strcmp(argv[2], "train")) train_detector(datacfg, cfg, weights, gpus, ngpus, clear,basecfg(cfg));
